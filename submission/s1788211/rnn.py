...
class RNN(object):
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            delta_out = d_hot - y[t]
            delta_in = self.W.T.dot(delta_out) * s[t] * (
                np.ones(self.hidden_dims) - s[t])
        ...
        for t in reversed(range(len(x))):
            ...
            for tau in range(1, steps + 1):
                if t >= tau:
                    ...
                    delta_in = self.U.T.dot(delta_in_prev) * s[t - tau] * (
                        np.ones(self.hidden_dims) - s[t - tau])
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        X_test, D_test = seqs_to_lmXY(X_test)
        mean_loss = my_rnn.compute_mean_loss(X_test, X_test)
        adjusted_loss = adjust_loss(mean_loss, fraction_lost, q)
        print("Mean loss: {}".format(mean_loss))
        print("Unadjusted: %.03f" % np.exp(mean_loss))
        print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))
    ...
    if mode == "train-np":
        ...
        np.save("./rnn.U (75 hidd, 10 steps, 2 learn_r, 25 epochs).npy",
                my_rnn.U)
        np.save("./rnn.V (75 hidd, 10 steps, 2 learn_r, 25 epochs).npy",
                my_rnn.V)
        np.save("./rnn.W (75 hidd, 10 steps, 2 learn_r, 25 epochs).npy",
                my_rnn.W)
